...
/**
 * Quota this source code in 2021-01-04.
 * Quota Reason: http://redmine.prod.dtstack.cn/issues/33981
 * Modify Content: adjust log level to debug when httpResponse.getHeaders has warning.
 *
...
final class RequestLogger {
    ...
    static void logResponse(Log logger, HttpUriRequest request, HttpHost host, HttpResponse httpResponse) {
        ...
        // adjust log level to debug when httpResponse.getHeaders has warning.
        if (logger.isDebugEnabled()) {
            Header[] warnings = httpResponse.getHeaders("Warning");
            if (warnings != null && warnings.length > 0) {
                logger.debug(buildWarningMessage(request, host, warnings));
            }
        }
