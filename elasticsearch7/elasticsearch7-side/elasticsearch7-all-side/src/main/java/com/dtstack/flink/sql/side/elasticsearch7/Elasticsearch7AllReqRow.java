 ...
 */
public class Elasticsearch7AllReqRow extends BaseAllReqRow implements Serializable {

    private static final Logger LOG = LoggerFactory.getLogger(Elasticsearch7AllReqRow.class);
    ...
    public Elasticsearch7AllReqRow(RowTypeInfo rowTypeInfo, JoinInfo joinInfo, List<FieldInfo> outFieldInfoList, AbstractSideTableInfo sideTableInfo) {
        super(new Elasticsearch7AllSideInfo(rowTypeInfo, joinInfo, outFieldInfoList, sideTableInfo));
    }
    ...
    protected void initCache() throws SQLException {
        ...
        try {
            ...
            boolQueryBuilder = Es7Util.setPredicateclause(sideInfo);
            loadData(newCache);
        } catch (Exception e) {
            LOG.error("", e);
    ...
    protected void reloadCache() {
        ...
        cacheRef.set(newCache);
        LOG.info("----- elasticsearch7 all cacheRef reload end:{}", Calendar.getInstance());
    }
    ...
    private void loadData(Map<String, List<Map<String, Object>>> tmpCache) throws IOException {
        ...
        try {
            for (int i = 0; i < CONN_RETRY_NUM; i++) {
                try {
                    rhlClient = Es7Util.getClient(tableInfo.getAddress(), tableInfo.isAuthMesh(), tableInfo.getUserName(), tableInfo.getPassword());
                    break;
                } catch (Exception e) {
    ...
    private void loadToCache(SearchHit[] searchHits, Map<String, List<Map<String, Object>>> tmpCache) {
        ...
        for (SearchHit searchHit : searchHits) {
            ...
            for (String fieldName : sideFieldNames) {
                ...
                object = Es7Util.getTarget(object, sideFieldTypes[fieldIndex]);
