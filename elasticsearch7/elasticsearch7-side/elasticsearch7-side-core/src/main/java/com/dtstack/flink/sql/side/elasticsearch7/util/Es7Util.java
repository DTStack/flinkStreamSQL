...
package com.dtstack.flink.sql.side.elasticsearch7.util;
 ...
 * @program: flink.sql
 * @author: lany
 * @create: 2021/01/11 10:23
 */
public class Es7Util {

    private static final Logger LOG = LoggerFactory.getLogger(Es7Util.class);
    ...
    public static RestHighLevelClient getClient(String esAddress, Boolean isAuthMesh, String userName, String password) {
        ...
        Arrays.stream(address).forEach( addr -> {
        ...
        if (LOG.isInfoEnabled()) {
            LOG.info("Create Elasticsearch RestHighLevelClient connected to {}.", httpHostList.toString());
        }

        return rhlClient;
    }

    // add index to search request
    ...
    public static BoolQueryBuilder setPredicateclause(BaseSideInfo sideInfo) {
        ...
        List<PredicateInfo> predicateInfos = sideInfo.getSideTableInfo().getPredicateInfoes();
        if (predicateInfos.size() > 0) {
            boolQueryBuilder = new BoolQueryBuilder();
            for (PredicateInfo info : predicateInfos) {
                boolQueryBuilder = Es7Util.buildFilterCondition(boolQueryBuilder, info, sideInfo);
            }
        }
