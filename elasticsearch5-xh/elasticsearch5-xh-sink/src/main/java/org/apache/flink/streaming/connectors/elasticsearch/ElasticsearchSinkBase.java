...
import transwarp.org.elasticsearch.action.ActionRequest;
import transwarp.org.elasticsearch.action.DocWriteRequest;
...
import transwarp.org.elasticsearch.action.bulk.BulkRequest;
...
import transwarp.org.elasticsearch.common.unit.ByteSizeUnit;
import transwarp.org.elasticsearch.common.unit.ByteSizeValue;
...
public abstract class ElasticsearchSinkBase<T, C extends AutoCloseable> extends RichSinkFunction<T> implements CheckpointedFunction {
	 ...
	 *
	 * <p>For {@link TransportClient} based implementations, this config
				...
				try {
					for (int i = 0; i < response.getItems().length; i++) {
						...
						if (failure != null) {
							...
							if (restStatus == null) {
								failureHandler.onFailure((ActionRequest) request.requests().get(i), failure, -1, failureRequestIndexer);
							} else {
								failureHandler.onFailure((ActionRequest) request.requests().get(i), failure, restStatus.getStatus(), failureRequestIndexer);
							}
						}
					}
				} catch (Throwable t) {
